...
def tracker(imagens):
	...
	for img in imagens:
		...
		for c in coords:
			part = hog(cv2.resize(img_grey[c[2]:c[3],c[0]:c[1]],resize_img),orientations=hog_orientations,pixels_per_cell=(hog_cell_size,hog_cell_size),cells_per_block=(hog_block_size,hog_block_size),block_norm="L2")
			Y = clf.predict(part.reshape((1,part.shape[0])))
